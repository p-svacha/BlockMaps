...
{
    public static class DefaultPathMeshBuilder
    {
        ...
        private static void DrawEdge(Direction dir, float xStart, float xEnd, float yStart, float yEnd)
        {
            ...
            bool hasPathConnection = HasSurfaceConnection(Node.GetSurface(), dir);
        ...
        private static void DrawCorner(Direction dir, float xStart, float xEnd, float yStart, float yEnd)
        {
            ...
            bool hasPathConnection = (
                HasSurfaceConnection(Node.GetSurface(), HelperFunctions.GetNextAnticlockwiseDirection8(dir)) &&
                HasSurfaceConnection(Node.GetSurface(), dir) &&
                HasSurfaceConnection(Node.GetSurface(), HelperFunctions.GetNextClockwiseDirection8(dir)));
        ...
        /// <summary>
        /// Checks and returns if a node with a specific surface exists in the given direction with a matching height to the current node.
        /// </summary>
        private static bool HasSurfaceConnection(Surface surface, Direction dir)
        {
            List<BlockmapNode> adjNodes = World.GetAdjacentNodes(Node.WorldCoordinates, dir);
            foreach (BlockmapNode adjNode in adjNodes)
                if (adjNode.GetSurface() == surface && World.DoAdjacentHeightsMatch(Node, adjNode, dir))
                    return true;
            return false;
        }
